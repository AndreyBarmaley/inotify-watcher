...
namespace SWE
{
    ...
    std::string escaped(const std::string & str, bool quote)
    {
    ...
    int JsonString::getInteger(void) const
    {
        ...
        {
            std::cerr << __FUNCTION__ << ": not number: " << content << std::endl;
        }
    ...
    bool JsonString::getBoolean(void) const
    {
        ...
        {
            std::cerr << __FUNCTION__ << ": not boolean: " << content << std::endl;
        }
    ...
    std::string JsonObject::toString(void) const
    {
        ...
        for(auto it = content.begin(); it != content.end(); ++it)
        {
            if((*it).second)
            {
                os << escaped((*it).first, true) << ": " << (*it).second->toString();
    ...
    bool JsonContent::parseBinary(const char* str, size_t len)
    {
        ...
        {
            std::cerr << __FUNCTION__ << ": invalid character inside JSON content" << std::endl;
            clear();
            return false;
        }
        else if(counts == JSMN_ERROR_PART)
        {
            std::cerr << __FUNCTION__ << ": the content is not a full JSON packet, more bytes expected" << std::endl;
            clear();
            return false;
        }
        else if(counts < 0)
    ...
    int JsonContent::pushValuesToObject(const const_iterator & it, JsonObject & obj) const
    {
        ...
        while(counts-- && itval != end())
        {
            if(!(*itkey).isKey())
            {
                ...
                std::cerr << __FUNCTION__ << ": not key, index: " << std::distance(begin(), itkey) << ", `"<< str << "'" << std::endl;
            }
    ...
    JsonContent::getValuePrimitive(const const_iterator & it) const
    {
        ...
        {
            if(lower(val).compare(0, 5, "false") == 0)
                return std::make_pair(JsonValuePtr(false), 1);

            if(lower(val).compare(0, 4, "true") == 0)
